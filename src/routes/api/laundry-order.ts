  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        await sendNotificationEmail(`Laundry quote request ${reference} — ${order.name}`, [
          ["Reference", reference],
          ["Customer", order.name],
          ["Email", order.email],
        ...
        await sendCustomerEmail(
          order.email,
          `We received your Nomad Laundry request — ${reference}`,
          `<div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;color:#25221d;line-height:1.6">
            <h1 style="font-size:26px">Your laundry request is in.</h1>
            ...
            <p>Nomad Laundry<br>A Knot &amp; Nomad Company</p>
          </div>`,
        );
